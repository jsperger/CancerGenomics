...
clinical.event <- GDCprepare_clinic(clinical.query, clinical.info = "new_tumor_event")
...
clinical <- merge(clinical, clinical.event, by= "bcr_patient_barcode" )
...
surv.vars <- c("bcr_patient_barcode", "days_to_death", "days_to_last_followup",
               ...
               "days_to_new_tumor_event_additional_surgery_procedure", "plat_use", "plat_type", "days_to_new_tumor_event_after_initial_treatment")
...
surv.data$EventTime <- pmin(surv.data$days_to_death, surv.data$days_to_new_tumor_event_after_initial_treatment, na.rm = TRUE)

# Create an indicator variable for whether the event is death or censoring
#surv.data$censored <- ifelse((!is.na(surv.data$days_to_death) | !is.na(surv.data$days_to_new_tumor_event_after_initial_treatment))  , 
#                              FALSE, 
#                              TRUE)

surv.data$censored <- ifelse(!(is.na(surv.data$EventTime)), TRUE, FALSE)

#Merge datasetes
cluster <- read.csv("subtype3_cluster_results.csv")
cluster$NMFC3 <- as.factor(cluster$NMFC3)

a <- merge(surv.data, cluster, by="bcr_patient_barcode")
...
######### Cox Model ######
#######################################
a <- surv.data[surv.data$plat_type == "Neither",]

#Cluster 2 is different from cluster 1 in terms of survival time
...
cox1 <- coxph(data=  surv.data, Surv(EventTime, censored) ~ age_at_initial_pathologic_diagnosis + NMFC3 + race_list   )
summary(cox1)

cox1 <- coxph(data=  surv.data, Surv(EventTime, censored) ~ age_at_initial_pathologic_diagnosis + NMFC3 + race_list + plat_use + NMFC3 * plat_use   )
summary(cox1)
